...
import packageManagerTest from '../../lib/package-manager/test.js';
...
test('npm-test: setup', async () => {
  ...
  await Promise.all([
    fsExtra.copy(passFixtures, passTemp),
    fsExtra.copy(failFixtures, failTemp),
    fsExtra.copy(badFixtures, badTemp),
    fsExtra.copy(scriptsFixtures, scriptsTemp),
    fsExtra.copy(writeTmpdirFixtures, writeTmpdirTemp)
  ]);
});
...
test('npm-test: tmpdir is redirected', async (t) => {
  ...
  await packageManagerTest('npm', context);
  t.ok(
    fsExtra.existsSync(join(writeTmpdirTemp, 'omg-i-write-to-tmpdir-testfile')),
